/** 
The user file for communicating between the clients and the MongoDB database.
...
// fetches past games
var past_games = function(db, user_name, callback) {
  user.get_past_games(db, user_name, function(err, res) {
    if(err) return;
    callback(res);
  });
};
